...
The aMDeNM simulations are done using *[OpenMM](https://openmm.org/)* and CHARMM36m forcefield. We strongly recommend that all system preparation be done with *[CHARMM-GUI](http://charmm-gui.org)*.
...
**Note:** `-s`/`--sel` and `-T`/`--temp` must stay the same across repeated `freeenergy` calls on the same simulation — see [Extending a Previous Calculation](#extending-a-previous-calculation).
...
## Output Files Description
1. **`run_metadata.json`**: the clustering selection, temperature, cutoff, de-excitation length, `max_centroids`, and production length used.
2. **`clustering_rmsd_cache.npz`/`.json`**: the pairwise-RMSD matrix over subsampled frames.
3. **`clustering_summary.csv`**: summary containing cluster ID, frame index, cluster size, status this run (fresh/extended/skipped), and cycles/ps completed.
4. **`fel_mode[N].csv`/`fel_mode[N]_plot.png`**: 1D free energy landscape per mode, in Å and kcal/mol.
5. **`fel_2d_mode[N]_mode[M].png`**: 2D free energy landscape for a mode pair.
6. **`fel_summary.html`**: interactive summary with protocol parameters, per-mode FEL statistics, per-centroid production status, and embedded plots.
[Back to top ↩](#)
* ****
...
```
python pyAdMD.py analyze -src freeenergy
```
## Compute a free energy landscape
```
python pyAdMD.py freeenergy -c 2 -p 100
```
## Extend a previous free energy calculation with more centroids and production time
```
python pyAdMD.py freeenergy -c 2 -p 500 --max_centroids 100
```
